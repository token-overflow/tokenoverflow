...
fn submit_schema_has_enhanced_descriptions() {
    ...
    let confirmed_desc = properties["confirmed"]["description"]
        .as_str()
        .expect("confirmed should have description");
    assert!(
        confirmed_desc.contains("AskUserQuestion"),
        "Confirmed description should mention AskUserQuestion"
    );
}
