...
| `integrations/claude/agents/tokenoverflow-researcher.md` | Exists | Remove the unsupported `mcpServers:` frontmatter (rejected by `--strict`). Swap the `tools:` whitelist for a `disallowedTools` denylist that blocks file mutation, shell execution, and web search; the agent inherits every other tool including the entire `mcp__tokenoverflow__*` surface. |
